 ...
 * corresponding struct, as generated by EnumVariantType macro.  Further, that struct must have a
 * TryFrom / TryInto implementation for the enum (as generated by EVT), and its own
...
pub fn evt_pyclass(item: TokenStream) -> TokenStream {
    ...
    let ident = ast.ident;
    let arms = de.variants.clone().into_iter().map(|v|v.ident).collect::<Vec<_>>();
    quote! {
